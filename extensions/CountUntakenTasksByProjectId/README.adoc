...
= CountUntakenTasksByProjectId
Devuelve cuantas tareas hay sin tomar de un proyecto

The project **CountUntakenTasksByProjectId** is a Bonita REST API extension for **Bonita {short-bonita-version}** version  **community** written in `java` language.
...
[source,bash]
----
git clone https://your.github.com/CountUntakenTasksByProjectId.git
cd CountUntakenTasksByProjectId/
mvn package
----

The build should produce a zip archive under the `target/` folder named `CountUntakenTasksByProjectId-0.0.1.zip`
...
Take the built artifact `CountUntakenTasksByProjectId-0.0.1.zip` and upload it to your Bonita platform (see {doc-url}/rest-api-extension-archetype#_deployment[rest-api-extension deployment])

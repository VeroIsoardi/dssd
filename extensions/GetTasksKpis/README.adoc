...
= GetTasksKpis
Lista data sobre las tareas

The project **GetTasksKpis** is a Bonita REST API extension for **Bonita {short-bonita-version}** version  **community** written in `java` language.
...
[source,bash]
----
git clone https://your.github.com/GetTasksKpis.git
cd GetTasksKpis/
mvn package
----

The build should produce a zip archive under the `target/` folder named `GetTasksKpis-0.0.1.zip`
...
Take the built artifact `GetTasksKpis-0.0.1.zip` and upload it to your Bonita platform (see {doc-url}/rest-api-extension-archetype#_deployment[rest-api-extension deployment])

...
/**
 * Get KPIs for tasks from cloud API.
 * Body must include: email, password (cloud admin credentials).
 * Calls `/tasks/kpis` and returns KPIs.
 */
public class Index extends AbstractIndex {
    ...
    protected Result execute(RestAPIContext context) {
        ...
        try {
            URI uri = URI.create(API_BASE_URL + "/tasks/kpis");
            ...
            JsonNode json = getMapper().readTree(body);

            return Result.builder()
                    .unTakenTasks(json.get("unTakenTasks").asInt())
                    .pendingTasks(json.get("pendingTasks").asInt())
                    .finishedTasks(json.get("finishedTasks").asInt())
                    .build();

        } catch (Exception e) {
            LOGGER.error("Failed to fetch task KPIs from external API", e);
            throw new RuntimeException("Failed to fetch task KPIs from external API", e);
        } finally {
            EMAIL.remove();
            PASSWORD.remove();
        }

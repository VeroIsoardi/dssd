...
/**
 * Controller class to assign/take a task in external cloud API.
 * Expects a JSON body with `taskId`, `email` and `name`.
 */
public class Index extends AbstractIndex {
    ...
    public void validateInputParameters(HttpServletRequest request) {
        // Read JSON body and extract taskId, email and name
        try {
            ...
            LOGGER.debug("Could not read taskId/email/name from request body: {}", e.getMessage());
        }
    }

    /**
     * Login to external API and obtain a bearer token using admin credentials
     */
    ...
    protected Result execute(RestAPIContext context) {
        ...
        LOGGER.info("Assign task request: taskId={}, email={}, emailCloud={}, name={}", taskId, email, emailCloud, name);

        if (taskId == null || taskId.isBlank()) {
            throw new RuntimeException("Missing taskId in request body");
        }

        // Obtain bearer token using emailCloud and password from request body
        ...
        try {
            // Build URL to take the task. Endpoint used: POST /tasks/{taskId}/take
            URI uri = URI.create(API_BASE_URL + "/tasks/" + taskId + "/take");

            String requestBody = format("{\"email\":\"%s\",\"name\":\"%s\"}",
                    email == null ? "" : email,
                    name == null ? "" : name);
            ...
            LOGGER.error("Failed to assign/take task in external API", e);
            throw new RuntimeException("Failed to assign/take task in external API", e);
        } finally {
            TASK_ID.remove();
            EMAIL.remove();
            EMAIL_CLOUD.remove();

...
The project **FinishTask** is a Bonita REST API extension for **Bonita {short-bonita-version}** version  **community** written in `java` language.
...
[source,bash]
----
git clone https://your.github.com/FinishTask.git
cd FinishTask/
mvn package
----

The build should produce a zip archive under the `target/` folder named `FinishTask-0.0.1.zip`
...
Take the built artifact `FinishTask-0.0.1.zip` and upload it to your Bonita platform (see {doc-url}/rest-api-extension-archetype#_deployment[rest-api-extension deployment])

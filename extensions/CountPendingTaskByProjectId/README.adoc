...
= CountPendingTaskByProjectId
Devuelve la cantidad de tareas pendientes para un projecto dado

The project **CountPendingTaskByProjectId** is a Bonita REST API extension for **Bonita {short-bonita-version}** version  **community** written in `java` language.
...
[source,bash]
----
git clone https://your.github.com/CountPendingTaskByProjectId.git
cd CountPendingTaskByProjectId/
mvn package
----

The build should produce a zip archive under the `target/` folder named `CountPendingTaskByProjectId-0.0.1.zip`
...
Take the built artifact `CountPendingTaskByProjectId-0.0.1.zip` and upload it to your Bonita platform (see {doc-url}/rest-api-extension-archetype#_deployment[rest-api-extension deployment])

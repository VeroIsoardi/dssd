...
= ListTasksByUser
Lista las tareas que tiene asignado un usuario

The project **ListTasksByUser** is a Bonita REST API extension for **Bonita {short-bonita-version}** version  **community** written in `java` language.
...
[source,bash]
----
git clone https://your.github.com/ListTasksByUser.git
cd ListTasksByUser/
mvn package
----

The build should produce a zip archive under the `target/` folder named `ListTasksByUser-0.0.1.zip`
...
Take the built artifact `ListTasksByUser-0.0.1.zip` and upload it to your Bonita platform (see {doc-url}/rest-api-extension-archetype#_deployment[rest-api-extension deployment])
